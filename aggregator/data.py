...
class infoDataset(Dataset):
    ...
    def get_input(self, index):
        ...
        scores = []
        for d in data:
            if self.type == '1_score':
                pred_score = [0.] * self.score_range
                ms = d['pred_score'][int(d['pred_label'])]
                if ms > 0.5:
                    pred_score[d['pred_label']] = ms
                    pred_score[1 - d['pred_label']] = 1. - ms
                else:
                    pred_score[d['pred_label']] = 1. - ms
                    pred_score[1 - d['pred_label']] = ms

            elif self.type == 'softmax':
                pred_score = F.softmax(torch.tensor(d['pred_score']), dim=0).tolist()
