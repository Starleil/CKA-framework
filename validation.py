...
from datasets.coco import build_keycan
from engine import evaluate, train_one_epoch_keycan, evaluate_each_cat_keycan
from our_models import build_model
from tensorboardX import SummaryWriter

import os

os.environ['CUDA_VISIBLE_DEVICES'] = "2" #opti-adam
...
def get_args_parser():
    ...
    parser.add_argument('--self_attn', default=True, action='store_true')  # False
    ...
    parser.add_argument('--sgd', default=True, action='store_true')  # False
    ...
    # our Backbone
    parser.add_argument('--sw_attn', default=True, action='store_true')  # False
    parser.add_argument('--window_size',
                        default=7,
                        type=int,
                        help="Window_size in the shift windows attention")
    parser.add_argument('--shift_size',
                        default=3,
                        type=int,
                        help="Shift_size in the shift windows attention")
    ...
    # our Transformer (addition)
    parser.add_argument(
        '--aux_nheads',
        default=4,  # 2
        type=int,
        help="Number of attention heads inside the vision transformer's attentions")
    ...
    parser.add_argument('--data_coco_lite_path', default='./our_buv', type=str)
    ...
    parser.add_argument('--output_dir',
                        default='./test_output(ourbuv_top300)',
                        help='path where to save, empty for no saving')
    parser.add_argument('--device',
                        default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--resume', default='./output(NT_ourbuv1016)/checkpoint0003.pth',
                        help='resume from checkpoint')  # ./output(NT_ourbuv1016)/checkpoint0003.pth ./output(NT_micbuv1120_lr1e-5)/checkpoint0002.pth
    parser.add_argument('--start_epoch',
                        ...
                        help='start epoch')
    # online
    parser.add_argument('--online_train', default=False, action='store_true')
    parser.add_argument('--online_keycan', default=True, action='store_true')
    parser.add_argument('--max_frm_id', default=0.99, type=float)
    #
...
def main(args):
    ...
    keycan_val = build_keycan(image_set='val', args=args)
    ...
    if args.resume:
        ...
        if not args.eval:
            test_stats, coco_evaluator, print_eachCat = evaluate_each_cat_keycan(
                model, criterion, postprocessors, data_loader_val, base_ds, device, keycan_val, args.output_dir, args.data_coco_lite_path
            )
            print(print_eachCat)
    ...
    #
    keycan_train = build_keycan(image_set='train', args=args)

    start_time = time.time()
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            sampler_train.set_epoch(epoch)
        train_stats = train_one_epoch_keycan(model, criterion, data_loader_train,
                                      optimizer, device, epoch, keycan_train,
        ...
        test_stats, coco_evaluator, print_eachCat = evaluate_each_cat_keycan(model, criterion, postprocessors,
                                                                      data_loader_val, base_ds, device, keycan_val,
                                                                      args.output_dir)
        ...
        log_stats = {
            ...
            **{f'testAP50_{k}': v
               for k, v in print_eachCat.items()}, 'epoch': epoch,
