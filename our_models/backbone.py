...
from .position_encoding_embedding import build_position_encoding
from .interframe_attention import GlobalInterFrameAttention, LocalInterFrameAttention
...
class Joiner(nn.Sequential):
    def __init__(self, backbone, position_embedding, self_attn=False):
        ...
        if self.self_attn:
            ...
            self.lifa_attn = LocalInterFrameAttention(d_model, n_heads, attn_drop=dropout)
            self.gifa_attn = GlobalInterFrameAttention(d_model, n_heads, attn_drop=dropout)
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        for i, (name, x) in enumerate(sorted(xs.items())):
            ...
            x.tensors = x.tensors.reshape(b, f, c, w, h) # 0: Lk, 1: Lk-1, 2:Lk+1, 3:Gk, 4:Gk-1, 5:Gk+1 # (1, 6, 256, w, h)

            # GIFA
            for i in range(f // 2):
                x1 = x.tensors[:, i, :, :, :].flatten(2).permute(2, 0, 1) # (w*h, 1, 256)
                x2 = x.tensors[:, i + f // 2, :, :, :].flatten(2).permute(2, 0, 1)
                
                x_i = self.gifa_attn(x1, x2)
                x_i = self.norm1(x_i)
                x_i = x1 + x_i
                xis.append(x_i)

            # LIFA
            xx = self.lifa_attn(xis[2], xis[1], xis[0])
            xx = xis[0] + xx
        ...
        for x in out:
            pos, pos_row, pos_col = self[1](x)
            poses.append(pos.to(x.tensors.dtype))
            pos_cols.append(pos_col.to(x.tensors.dtype))
            pos_rows.append(pos_row.to(x.tensors.dtype))

        return out, poses, pos_cols, pos_rows


def build_backbone(args):
    position_embedding = build_position_encoding(args)
    # position_embedding = RCPositionEmbeddingLearned(args.hidden_dim // 2)
